(function () {
  ...
  const state = {
    ...
    studioPageNav: null,
    studioPageHost: null,
    studioPageNavTabs: null,
    studioPageNavTabsHandler: null,
    studioPageId: "",
      ...
      while (queue.length) {
        ...
        if (!globalStats) {
          try {
            ...
            console.warn("[StudioDashboard] Performer global stats failed", id, err);
            continue;
          }
        }
  ...
  function createPageSection(container, titleText, options = {}) {
    ...
    if (titleText) {
      ...
      main.addEventListener("click", (event) => {
        if (event.target instanceof Element && event.target.closest(".studio-dashboard__page-section-actions")) return;
  ...
  function removeStudioPageDashboard() {
    if (state.studioPageNavTabs && state.studioPageNavTabsHandler) {
      state.studioPageNavTabs.removeEventListener("click", state.studioPageNavTabsHandler);
    }
    state.studioPageNav?.remove();
    state.studioPageHost?.remove();
    state.studioPageNav = null;
    state.studioPageHost = null;
    state.studioPageNavTabs = null;
    state.studioPageNavTabsHandler = null;
    state.studioPageId = "";
  }
  ...
  function ensureStudioPageDashboard() {
    ...
    mount.navTabs.addEventListener("click", navTabsHandler);
    state.studioPageNav = nav;
    state.studioPageHost = host;
    state.studioPageNavTabs = mount.navTabs;
    state.studioPageNavTabsHandler = navTabsHandler;
    state.studioPageId = studioId;
  }
  ...
  function installObserver() {
    ...
    state.observer = new MutationObserver((records) => {
      const shouldEnhance = records.some((record) => {
        ...
        return !target?.closest(".studio-dashboard__page-dashboard, .studio-dashboard__dashboard-tab-link, .studio-dashboard__tooltip");
      });
